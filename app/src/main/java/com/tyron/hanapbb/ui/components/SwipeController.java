...
public class SwipeController extends ItemTouchHelper.Callback {
    ...
    public SwipeController(Context context, ISwipeControllerActions swipeControllerActions){
        ...
        mReplyIcon = ContextCompat.getDrawable(mContext,R.drawable.ic_reply_black_24dp);
        mReplyIconBackground = ResourcesCompat.getDrawable(mContext.getResources(), R.drawable.ic_round_shape,null);
    }
    ...
    public int getMovementFlags(@NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder) {
        ...
        if(viewHolder.getItemViewType() == ChatAdapter.VIEW_TYPE_EMPTY) return 0;
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive){
        ...
        if (mView.getTranslationX() < convertToDp(130) || dX > mDx ){
            ...
            float width = AndroidUtilities.getScreenWidth();
    ...
    private void setTouchListener(RecyclerView recyclerView, final RecyclerView.ViewHolder viewHolder){
        if(viewHolder.getItemViewType() != ChatAdapter.VIEW_TYPE_EMPTY) {
    ...
    private int convertToDp(int pixels){
        return AndroidUtilities.dp(mContext, pixels);
    }


    private void drawReplyButton(Canvas canvas){

        int width = (int)AndroidUtilities.getScreenWidth();
