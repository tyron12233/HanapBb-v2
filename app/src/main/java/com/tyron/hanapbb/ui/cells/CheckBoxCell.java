...
import com.tyron.hanapbb.ui.components.CheckBox2;
import com.tyron.hanapbb.ui.components.CheckBoxSquare;
...
public class CheckBoxCell extends FrameLayout {
    ...
    public CheckBoxCell(Context context, int type, int padding) {
        ...
        if (type == 3) {
            ...
            if (type == 2) {
                addView(textView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT, (BottomSheet.LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT) | Gravity.TOP, (BottomSheet.LocaleController.isRTL ? 0 : 29), 0, (BottomSheet.LocaleController.isRTL ? 29 : 0), 0));
            } else {
                int offset = type == 4 ? 56 : 46;
                addView(textView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT, (BottomSheet.LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT) | Gravity.TOP, (BottomSheet.LocaleController.isRTL ? padding : offset + (padding - 17)), 0, (BottomSheet.LocaleController.isRTL ? offset + (padding - 17) : padding), 0));
            }
        }
        ...
        valueTextView.setGravity((BottomSheet.LocaleController.isRTL ? Gravity.LEFT : Gravity.RIGHT) | Gravity.CENTER_VERTICAL);
        addView(valueTextView, LayoutHelper.createFrame(LayoutHelper.WRAP_CONTENT, LayoutHelper.MATCH_PARENT, (BottomSheet.LocaleController.isRTL ? Gravity.LEFT : Gravity.RIGHT) | Gravity.TOP, padding, 0, padding, 0));
        ...
        if (type == TYPE_CHECK_BOX_ROUND) {
            ...
            checkBoxSize = 21;
            addView(checkBox, LayoutHelper.createFrame(checkBoxSize, checkBoxSize, (BottomSheet.LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT) | Gravity.TOP, (BottomSheet.LocaleController.isRTL ? 0 : padding), 16, (BottomSheet.LocaleController.isRTL ? padding : 0), 0));
        } else {
            ...
            if (type == 3) {
                ...
                addView(checkBox, LayoutHelper.createFrame(checkBoxSize, checkBoxSize, (BottomSheet.LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT) | Gravity.TOP, 0, 15, 0, 0));
            } else {
                addView(checkBox, LayoutHelper.createFrame(checkBoxSize, checkBoxSize, (BottomSheet.LocaleController.isRTL ? Gravity.RIGHT : Gravity.LEFT) | Gravity.TOP, (BottomSheet.LocaleController.isRTL ? 0 : padding), 16, (BottomSheet.LocaleController.isRTL ? padding : 0), 0));
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        if (needDivider) {
            ...
            canvas.drawLine(BottomSheet.LocaleController.isRTL ? 0 : AndroidUtilities.dp(offset), getMeasuredHeight() - 1, getMeasuredWidth() - (BottomSheet.LocaleController.isRTL ? AndroidUtilities.dp(offset) : 0), getMeasuredHeight() - 1, Theme.dividerPaint);
        }
    }
